...
<div class="layout">
    <div class="topbar">
        <div>
            <h1>Panel de empleados: generacion de pases</h1>
            <div class="muted">Emite pases y consulta solamente los pases que has generado.</div>
        </div>
        <div class="topbar-actions">
            <div class="muted">Sesion: <strong><?= htmlspecialchars($currentDisplayName, ENT_QUOTES, 'UTF-8') ?></strong> (<?= htmlspecialchars($currentEmployeeUid, ENT_QUOTES, 'UTF-8') ?>)</div>
    ...
    <section class="card">
        ...
        <form method="post" class="stack-sm">
            ...
            <div class="form-grid">
                ...
                <div>
                    <label for="issued_by_employee_uid_locked">Empleado autenticado</label>
                    <input id="issued_by_employee_uid_locked" type="text" value="<?= htmlspecialchars($currentDisplayName . ' (' . $currentEmployeeUid . ')', ENT_QUOTES, 'UTF-8') ?>" readonly>
                </div>
            </div>
...
(() => {
    function setupQrButton({buttonId, sourceCodeIdId, containerId, downloadButtonId}) {
        ...
        if (!btnGenerate || !codeElement || !container || !btnDownload || typeof QRCode === 'undefined') {
            return;
        }
    ...
    document.querySelectorAll('.js-generate-table-qr').forEach((button) => {
        button.addEventListener('click', () => {
            if (!tableQrContainer || !tableQrDownloadBtn || typeof QRCode === 'undefined') {
                return;
            }
    ...
    if (tableQrDownloadBtn) {
        tableQrDownloadBtn.addEventListener('click', () => {
            ...
            const anchor = document.createElement('a');
            anchor.href = url;
            anchor.download = `hacceso-pass-${lastTableCodeId}.png`;
            anchor.click();
        });
    }
})();
